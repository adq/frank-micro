...
static inline void dvi_serialiser_program_init(PIO pio, uint sm, uint offset, uint data_pins, bool debug) {
    ...
    /* PATCH (frank-hdmi-sound): when sys_clock runs at an integer multiple
     * of the TMDS bit clock (e.g. 504 MHz CPU vs 252 MHz TMDS), apply
     * the matching clkdiv to the serialiser SM so TMDS comes out at
     * spec rate without forcing the CPU down to TMDS speed. */
#ifdef DVI_SM_CLKDIV
    sm_config_set_clkdiv(&c, DVI_SM_CLKDIV);
#endif
