...
check_r7:
    ...
    if (p_video->is_rendering_active) {
      ...
      if (p_video->is_interlace_sync_and_video) {
         ...
         */
#ifdef PICO_BUILD
        /* The Pico renderer is progressive (single 250-line field), so we
         * pin the SAA5050 to the even field.  Feeding the alternating
         * is_odd_frame here makes teletext glyph rows shift every frame,
         * which shows up as character "shape flicker" on a progressive
         * display. */
