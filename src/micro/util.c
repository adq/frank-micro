...
#ifdef PICO_BUILD
#include <stdint.h>
/* On RP2350 the disc images (~2.2 MB each) and other large beebjit
 * allocations cannot fit in 512 KB SRAM, so route big blocks to the
 * memory-mapped PSRAM heap.  Small allocations stay in fast SRAM.
 * PSRAM is mapped at the XIP window 0x11000000; SRAM is at 0x20000000,
 * so the source region is unambiguous from the pointer value. */
...
#ifdef PICO_BUILD
  if (size >= PICO_PSRAM_THRESHOLD && psram_is_available()) {
    void* p_psram = psram_malloc(size);
    if (p_psram != NULL) {
      return p_psram;
    }
    /* Fall through to SRAM if PSRAM heap is exhausted. */
  }
#endif
...
#ifdef PICO_BUILD
  if (util_is_psram_ptr(p)) {
    void* p_ret = psram_realloc(p, size);
    if (p_ret == NULL) {
...
#ifndef PICO_BUILD  /* util_file_* replaced by util_file_pico.c */
struct util_file*
...
#ifdef PICO_BUILD
  /* On RP2350: crash immediately via hard fault so the crash handler
   * saves the PC and reboots.  NO printf — USB CDC would block forever. */
  asm volatile("udf #0");
  while (1) { }
#else
