...
#ifdef PICO_BUILD
  /* Disc build is a long operation (2.2MB PSRAM writes) — feed the watchdog
   * so it doesn't fire before the disc is ready. */
  extern void crash_handler_feed(void);
  crash_handler_feed();
#endif
...
#ifdef PICO_BUILD
  crash_handler_feed();  /* feed again after load completes */
#endif
}
