...
#ifdef PICO_BUILD
  /* On RP2350 the BBC CPU thread (Core 1) calls this.
   * We call pico_vsync_handler() which flips the framebuffer and reads
   * keyboard/serial from Core 0 context — no channel blocking needed. */
  extern void pico_vsync_handler(int do_full_render);
  pico_vsync_handler(do_full_render);
...
bbc_create(int mode,
   ...
   */
#ifdef PICO_BUILD
  /* On RP2350 we don't need the Windows boundary trick — use flat 64k region. */
  map_size = k_6502_addr_space_size;
  half_map_size = 0;
  map_offset = 0;
#else
  map_size = (k_6502_addr_space_size * 2);
...
#ifdef PICO_BUILD
  /* On RP2350 allocate full 16×16KB sideways ROM bank from PSRAM. */
  extern uint8_t* pico_alloc_sw_roms(size_t size);
  p_bbc->p_mem_sideways = pico_alloc_sw_roms(k_bbc_rom_size * k_bbc_num_roms);
  if (!p_bbc->p_mem_sideways) {
    /* PSRAM unavailable — fall back to SRAM heap */
